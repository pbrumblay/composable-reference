...
const useCacheHandler = {
    async get(cacheKey, softTags) {
        console.log('[use-cache-handler] process.env:', process.env);
        console.log('[use-cache-handler] get: SKIP_USE_CACHE_DURING_BUILD:', process.env.SKIP_USE_CACHE_DURING_BUILD);
        if (process.env.SKIP_USE_CACHE_DURING_BUILD === '1') {
            debug('SKIP', cacheKey, '(SKIP_USE_CACHE_DURING_BUILD)');
            return undefined;
        }
